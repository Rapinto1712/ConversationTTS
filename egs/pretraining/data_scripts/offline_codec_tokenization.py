...
def get_parser():
    ...
    parser.add_argument("--parquet-scp", type=str, default=None, help="input parquet file list")
...
def main(args):
    ...
    if args.output_utt2spk is not None:
    ...
    elif args.parquet_scp is not None:
        # parquet format
        with open(args.parquet_scp, 'r') as f:
            lines = f.readlines()
            for i, line in enumerate(lines):
                parquet_path = line.strip().split(' ')[-1] # get the parquet path
                ...
                for i_r, row in df.iterrows():
                    ...
                    wav = torch.from_numpy(audio_data).T[:1, :].float() # transfer to (1,len)
                    if wav.shape[1] / sample_rate > 40:
                        continue
                    utts.append((wav, sample_rate))
                    text = row['transcript_sensevoice']
                    f_text.write(key+' '+text+'\n')
                    names.append(key)
                    ...
                    data_dict[key] = value
                    # print(value.shape)
                    if i_r > 0 and i_r % 1000 == 0:
                        logging.info(f"processed {i_r} examples")
                
                logging.info(f"Parquet {parquet_path} processed")
    else:
        # kaldiio format
